...
function get_experiment(client::Client, id::Integer)::Optional{Experiment}
    try
        return _json(_request(client, "GET", "/experiment/$id")) |> Experiment
    catch err
...
[`Experiment`](@ref) records under `project_id` and discards the pagination envelope.
"""
function get_experiments(
    client::Client, project_id::Integer,
)::Array{Experiment,1}
...
function get_experiments(
    ...
    response = _request(
        client, "GET", "/experiment/project/$project_id";
...
`status_id` must equal `Integer(IN_PROGRESS)` — the server rejects experiments that are
created already terminated. Requires [`CreatePermission`](@ref) on the project. Returns
...
function create_experiment(
    ...
    response = _request(
        client, "POST", "/experiment/project/$project_id";
