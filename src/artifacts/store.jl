...
([`SQLiteStore`](@ref), [`FilesystemStore`](@ref), [`S3Store`](@ref)) implement
...
- `uri::String`: Canonical pointer at the stored bytes. The empty string is the sentinel for
  the SQLite backend (the bytes live inline in `resource.data`).
...
Return the short string identifier (`"sqlite"`, `"filesystem"`, `"s3"`) used to populate
...
upload. For the [`SQLiteStore`](@ref) it is a no-op write — the bytes still travel into the
...
[`SQLiteStore`](@ref) returns it directly.
"""
...
For the [`SQLiteStore`](@ref) this is a no-op (the bytes vanish when the parent
...
"""
sha256_hex(data::AbstractVector{UInt8})::String = data |> SHA.sha256 |> bytes2hex
...
the function falls back to [`SQLiteStore`](@ref) so offline code paths (`@with_deardiary_test_db`)
...
function current_artifact_store()::AbstractArtifactStore
    ...
    if _DEARDIARY_APICONFIG |> isnothing
        return SQLiteStore()
    end
...
fall back to [`SQLiteStore`](@ref) with a warning — the server stays bootable, but the
...
function artifact_store_for(config::APIConfig)::AbstractArtifactStore
    ...
    @warn "Unknown DEARDIARY_ARTIFACT_BACKEND '$(backend)' — falling back to sqlite"
    return SQLiteStore()
end
...
Backend-only convenience: returns [`SQLiteStore`](@ref) for `"sqlite"` and warns otherwise.
...
function artifact_store_for(backend::AbstractString)::AbstractArtifactStore
    ...
    @warn "artifact_store_for($(backend)) needs an APIConfig — falling back to sqlite"
    return SQLiteStore()
end
