...
- `skipped::Int64`: Number of rows that were already on a non-SQLite backend at scan time
...
Walk every [`Resource`](@ref) row whose `backend == "sqlite"`, replay its inline bytes
through `target`, and update the row to point at the new artifact. Designed for one-shot
backfill after switching `DEARDIARY_ARTIFACT_BACKEND` away from `"sqlite"`:
...
Refuses to do anything when `target` is itself a [`SQLiteStore`](@ref) — there is nowhere to
move the bytes to.
...
function migrate_artifacts!(
    ...
    if target isa SQLiteStore
        @warn "migrate_artifacts!: target is SQLiteStore — nothing to do"
        return MigrateArtifactsResult(0, 0, 0)
    end

    db = get_database()
    rows = DBInterface.execute(
        db, "SELECT id FROM resource WHERE backend = 'sqlite' ORDER BY id",
    )
    ids = [row.id for row in rows]

    target_backend = target |> backend_id
    migrated = 0
    skipped = 0
    failed = 0
    ...
    for id in ids
        ...
        if (resource |> isnothing) || resource.backend != "sqlite"
            skipped += 1
            continue
        end

        bytes = (resource.data |> isnothing) ? UInt8[] : (resource.data |> Vector{UInt8})
        try
            write_result = write_artifact(target, bytes)
            # Commit the move in a single UPDATE: flip the backend, point at the new URI,
            # blank out the inline bytes (the schema forbids NULL on `data`, so use an
            # empty BLOB instead), and stamp the metadata columns.
            DBInterface.execute(
                db,
                """
                UPDATE resource
    ...
    @info(
        ...
        migrated=migrated, skipped=skipped, failed=failed, target=target_backend,
    )
