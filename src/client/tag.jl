"""
    get_tag(client::Client, id::AbstractString)::Optional{Tag}
...
The value-based local lookup ([`get_tag_by_value`](@ref)) has no REST counterpart; iterate the
parent-scoped [`get_tags`](@ref) listings to discover a tag by its value.
"""
function get_tag(client::Client, id::AbstractString)::Optional{Tag}
    try
...
"""
    get_tags(client::Client, ::Type{T}, parent_id::AbstractString)::Array{Tag,1} where {T<:Union{Project,Experiment,Iteration}}
...
"""
function get_tags(
    client::Client, ::Type{T}, parent_id::AbstractString
...
"""
    add_tag(client::Client, ::Type{T}, parent_id::AbstractString, value::AbstractString)::String where {T<:Union{Project,Experiment,Iteration}}
...
"""
function add_tag(
    client::Client, ::Type{T}, parent_id::AbstractString, value::AbstractString
)::String where {T<:Union{Project,Experiment,Iteration}}
    response = _request(
