@with_deardiary_test_db begin
    @testset verbose = true "parameter repository" begin
        ...
        @testset verbose = true "fetch" begin
            @testset "existing parameter" begin
                ...
                parameter_id, _ = DearDiary.insert(
                    DearDiary.Parameter, iteration_id, "batch_size", "32"
                )
        ...
        @testset verbose = true "fetch all" begin
            ...
            iteration_id, _ = DearDiary.create_iteration(experiment_id)
            DearDiary.insert(DearDiary.Parameter, iteration_id, "dropout_rate", "0.5")
            DearDiary.insert(DearDiary.Parameter, iteration_id, "momentum", "0.9")
        ...
        @testset verbose = true "update" begin
            ...
            iteration_id, _ = DearDiary.create_iteration(experiment_id)
            parameter_id, _ = DearDiary.insert(
                DearDiary.Parameter, iteration_id, "weight_decay", "0.0001"
            )

            update_result = DearDiary.update(
                DearDiary.Parameter, parameter_id; value="0.0005"
            )
        ...
        @testset verbose = true "delete" begin
            @testset "single parameter" begin
                ...
                iteration_id, _ = DearDiary.create_iteration(experiment_id)
                parameter_id, _ = DearDiary.insert(
                    DearDiary.Parameter, iteration_id, "activation_function", "relu"
                )

                @test DearDiary.delete(DearDiary.Parameter, parameter_id)
                @test isnothing(DearDiary.fetch(DearDiary.Parameter, parameter_id))
            end
            ...
            @testset "all parameters by iteration" begin
                ...
                DearDiary.insert(DearDiary.Parameter, iteration_id, "optimizer", "adam")
                DearDiary.insert(
                    DearDiary.Parameter, iteration_id, "loss_function", "cross_entropy"
                )
