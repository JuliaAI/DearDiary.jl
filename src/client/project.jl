"""
    get_project(client::Client, id::AbstractString)::Optional{Project}
...
"""
function get_project(client::Client, id::AbstractString)::Optional{Project}
    try
...
"""
    create_project(client::Client, name::AbstractString)::String
...
"""
function create_project(client::Client, name::AbstractString)::String
    response = _request(client, "POST", "/project/"; body=Dict("name" => name))
    return _json(response)["project_id"]
end

"""
    update_project(client::Client, id::AbstractString; name=nothing, description=nothing)::Nothing
